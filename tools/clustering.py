...
from tools.dtw import BSDBA, DTW
...
class Kmeans(BaseEstimator, ClusterMixin):  # tested valid
    ...
    def _initialization(self, X: np.ndarray) -> None:
        ...
        while len(cluster_id) < self.ncluster:
            ...
            dist_to_cluster[:, len(cluster_id) - 1] = cdist(
                self.measure, X, to_time_series_dataset(temp_centroid), njobs=self.njobs
            ).flatten()
        ...
        init_centroid = [
            resample(ts.reshape(-1), self.centroid_size) for ts in X[cluster_id]
        ]
    ...
    def _update_centroid(self, X: np.ndarray) -> None:  # tested Valid
        ...
        for k, centroid in enumerate(self.centroid_):
            self.centroid_[k] = self.barycenter.transform(
                X[self.labels_ == k], centroid
            )
    ...
    def fit(self, X: np.ndarray) -> None:
        ...
        while (not success) and (n_attempt < max_attempt):
            try:
                ...
                if self.verbose:
                    print(
                        f"Resumed because of an empty cluster. Attempt {n_attempt}/{max_attempt}"
                    )
                n_attempt += 1
    ...
    def _one_try_fit(self, X: np.ndarray) -> None:  # tested valid
        ...
        for i in range(self.n_iteration):
            ...
            if self.verbose:
                print(
                    f"Iteration : {i+1}/{self.n_iteration} -- Inertia: {self.inertia_[-1]}"
                )
    ...
    def _compute_inertia(self) -> float:  # tested Valid
        return (
            np.sum(self.distances_[np.arange(self.nsamples_), self.labels_] ** 2)
            / self.nsamples_
        )
...
class KmeanDTW(Kmeans):  # tested valid
    ...
    def __init__(
        ...
        super().__init__(
            ncluster, self.dtw, self.bar_dtw, centroid_size, n_iteration, njobs, verbose
        )
    ...
    def fit(self, X):  # tested valid
        ...
        for i in range(self.ncluster):
            cluster_avg_lengths.append(
                np.mean([len(ts) for ts in X[self.labels_ == i]])
            )
        ...
        if self.quantile_threshold is not None:
            ...
            for i in range(self.ncluster):
                ...
                threshold_dists.append(
                    np.quantile(distance_label_set, self.quantile_threshold)
                )

            self.threshold_dists_ = np.array(threshold_dists).astype(float)
            thresholds = self.threshold_dists_[self.labels_]
            idx = (
                self.distances_[np.arange(self.labels_.shape[0]), self.labels_]
                > thresholds
            )
