...
from tools.base_dtw import (
    cost_matrix,
    single_valence_vector,
    single_warping_vector,
    warping_path,
)
...
class BSDBA(object):
    ...
    def transform(self, X: np.ndarray, init_barycenter: np.ndarray) -> np.ndarray:
        ...
        for it in range(self.max_iter):
            ...
            for batch in batches:
                ...
                subgradient = self._subgradient_step(
                    X[batch], paths, barycenter, b_size
                )
    ...
    def _paths_cost_barycenter(self, X: np.ndarray, barycenter: np.ndarray) -> list:
        ...
        """
        paths_cost_set = Parallel(self.njobs)(
            delayed(self.metric.warping_path_and_distance)(
                barycenter, X_i.astype(float)
            )
            for X_i in X
        )
    ...
    def _subgradient_step(
        self, X: np.ndarray, paths: list, barycenter: np.ndarray, barycenter_size: int
    ) -> np.ndarray:
        ...
        """
        valence_vectors = Parallel(self.njobs)(
            delayed(single_valence_vector)(path, barycenter_size) for path in paths
        )
        valence_vectors = np.array(valence_vectors)
        valence_vetor = np.sum(valence_vectors, axis=0) * barycenter

        warping_vectors = Parallel(self.njobs)(
            delayed(single_warping_vector)(path, ts.astype(float), barycenter_size)
            for path, ts in zip(paths, X)
        )
