...
class Pipeline(object):
    ...
    def __init__(
        ...
        prominence: float,
        wlen: float,
        cycle_minimum_duration: float,
        cycle_maximum_duration: float,
        training_size_per_interval: int,
        interval: int,
        in_ncluster: int,
        in_centroid_duration: int,
        out_ncluster: int,
        out_centroid_duration: int,
        down_sampfreq=None,
        radius=-1,
        n_iteration=10,
    ...
    def _segmentation(self, X: np.ndarray) -> None:
        ...
        for ts in X:
            phs = PhaseSeg(
                ...
                self.prominence,
                self.wlen,
                self.cycle_minimum_duration,
                self.cycle_maximum_duration,
                self.training_size_per_interval,
                self.interval,
            )
            phs.fit(ts)
            seg_lst.append(phs)
    ...
    def _predict(self):
        ...
        for phs in self.seg_lst:
            ...
            # append json pred.
            tdata = np.vstack(
                [phs.insp_start_[:-1], in_labels, phs.exp_start_, out_labels]
            ).T
            tdf = pd.DataFrame(
                tdata,
                columns=[
    ...
    def fit(self, X: np.ndarray) -> None:
        ...
        if self.verbose:
            print(" ### Inhalation ###")
        self.in_kdtw_ = KmeanDTW(
            self.in_ncluster,
            int(self.in_centroid_duration * self.freq_),
            int(self.radius * self.freq_),
            self.n_iteration,
        ...
        if self.verbose:
            print("\n ### Exhalation ###")
        self.out_kdtw_ = KmeanDTW(
            self.out_ncluster,
            int(self.out_centroid_duration * self.freq_),
            int(self.radius * self.freq_),
            self.n_iteration,
            self.quantile_threshold,
            self.njobs,
    ...
    def plot_intertia(self):
        fig = make_subplots(
            ...
            2,
            subplot_titles=[
                "Inhalation Inertia Convergence",
                "Exhalation Inertia Convergence",
            ],
        )
        fig.add_trace(go.Scatter(y=self.in_kdtw_.inertia_), row=1, col=1)
        fig.add_trace(go.Scatter(y=self.out_kdtw_.inertia_), row=1, col=2)
        fig.update_layout(
    ...
    def plot_medoid(self, in_palette="autumn", out_palette="winter"):
        ...
        in_colors = [
            f"rgb({int(x[0])},{int(x[1])},{int(x[2])})" for x in in_colors * 256
        ]
        out_cmap = plt.cm.get_cmap(out_palette, self.out_ncluster)
        out_colors = np.array([out_cmap(i) for i in range(self.out_ncluster)])
        out_colors = [
            f"rgb({int(x[0])},{int(x[1])},{int(x[2])})" for x in out_colors * 256
        ]

        in_labels = self.in_kdtw_.labels_
        out_labels = self.out_kdtw_.labels_
        in_distances = self.in_kdtw_.distances_[
            np.arange(n_samples), self.in_kdtw_.labels_
        ]
        out_distances = self.out_kdtw_.distances_[
            np.arange(n_samples), self.out_kdtw_.labels_
        ]
        ...
        for i in range(self.in_ncluster):
            for j in range(self.out_ncluster):
                if idx[i, j] != -1:
                    ...
                    fig.add_trace(
                        go.Scatter(
                            x=np.arange(ts_in.shape[0]) / self.freq_,
                            y=ts_in,
                            mode="lines",
                    ...
                    fig.add_trace(
                        go.Scatter(
                            x=np.arange(
                                ts_in.shape[0] - 1, ts_in.shape[0] - 1 + ts_out.shape[0]
                            )
                            / self.freq_,
                            y=ts_out,
                            mode="lines",
                            marker=dict(color=out_colors[j]),
                        ),
                        row=i + 1,
                        col=j + 1,
                    )
